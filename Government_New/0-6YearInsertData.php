...
include 'db.php';
	if(isset($_POST['save']))
	{
		...
		$result2=mysql_query("select childid from childrendetails where uid='".$_COOKIE['uid']."' order by childid desc limit 1");
		$hid=mysql_fetch_array($result2);
			$q=$_POST['answer'];
			foreach($q as $question)
			{
				//$result=mysql_query("insert into questionanswer(qid,answer,ChildId) values('".substr($question,1,10)."','".substr($question,0,1)."','".$hid[0]."')");
				echo "Question ID : ".substr($question,1,10);
				echo "<br>Answers : ".substr($question,0,1);
			}
			...
			<form name=f1 method=post>
			 ...
			 <div class="panel" style='border:1px solid #FF5050;color:white;radius:0px'>
					...
					<div class="panel-heading" style='background-color:#FF5050;radius:0px'>Questions</div>
						...
						$sql="select qid,question from question where cid='2' or cid='3' or cid='4'";
